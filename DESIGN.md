...
## Future Improvements: 
LazyReader has very limited functionality in this release version. Currently there is no support for word replacement other than nouns (non-proper ones). The main reasoning behind this is POS words like verbs require us to identify and then apply the correct verb tense to the replacement word. While OpenNLP does identity the verb tense, we did not have time to implement a verb tense modifier for the synonym word as well as the fact that OpenNLP does not contain functionality to do this. However, the Stanford University NLP API contains functionality that allows for verb tense conversion that can be utilized in the future and we may implement this API in the future (although it is more than a 500Mb API and GitHub may not allow it to be pushed but the steps to download it can be provided). We have been programming with thought of the functionality behind future expansion and our program should be easily be able to take in a seperate word difficulty/frequency bank such as the ones from https://www.wordfrequency.info/ which include 60k+ words compared to our 30k+ words from http://www.newgeneralservicelist.org/ngsls, our DifficultyClassifier can also scale with more or less words as well. Implementing verb tense conversion has also been accounted for as we have set up the POS tag conversions at each step to be able to account for verbs once that functionality is needed. Additionally, future releases of this program will include additional difficulty test functionality where a user will be able to take additional exams with words to more precisely guauge their reading level dropping the overall standard deviation of the range. Utilizing this data, we will be able to more accurately select appropriate words to replace based on the user. Hopefully with more sophisticated models and larger databases, LazyReader will be able to reach it's full potentially as an app for people to use in the future.
